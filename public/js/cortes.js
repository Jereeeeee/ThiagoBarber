document.addEventListener('DOMContentLoaded', () => {
    const page = document.querySelector('[data-cortes-page]');

    if (!page) {
        return;
    }

    const config = parseConfig(page.dataset.cortesConfig);
    ...
    openEditButtons.forEach((button) => {
        button.addEventListener('click', () => {
            ...
            if (!editForm || !editTitleInput || !editPreviewImage || !editCorteIdInput) {
                return;
            }

            editForm.action = buildRoute(routeTemplates.update, corteId);
            editTitleInput.value = button.dataset.corteTitle || '';
            editPreviewImage.src = button.dataset.corteImage || placeholderImage;
    ...
    openDeleteButtons.forEach((button) => {
        button.addEventListener('click', () => {
            ...
            deleteForm.action = buildRoute(routeTemplates.destroy, corteId);
            deleteTitle.textContent = button.dataset.corteTitle || 'seleccionada';
    ...
    if (config.shouldOpenUpdateModal && config.oldCorteId) {
        const oldCorteButton = document.querySelector('[data-open-edit-modal][data-corte-id="' + config.oldCorteId + '"]');
